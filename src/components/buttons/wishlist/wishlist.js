...
import { WishlistIconStyles } from "./wishlistIconStyles";
import { getProduct } from "../../../functions/getProduct";
import data from "../../../data.json";

export const WishlistIcon = ({ id }) => {
  const { wish } = useSelector((state) => ({ ...state }));

  let wishProduct = getProduct(data, id).find((el) => el);
  console.log("wishProduct", wish);

  const showIcon = (id) => {
    let icon = {};
    for (let i = 0; i < wish.length; i++) {
      console.log("wish[i].id", wish[i].id);
      console.log("wishProduct", wishProduct);

      if (wish[i].id === wishProduct.id) {
        icon = (
          <Icon red="false" viewBox="-6 -5.5 28 28">
